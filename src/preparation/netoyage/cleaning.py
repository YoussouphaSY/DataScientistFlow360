...
def create_db_and_table():
    ...
    try:
        ...
        """
        cur.execute(create_table_sql)
        logging.info(f"✅ Table {POSTGRES['table']} prête")
        cur.close()
        conn.close()
    except Exception as e:
        logging.error(f"❌ Erreur création DB/table: {e}")
...
def save_to_postgres(df, mode="append"):
    """Sauvegarde le DataFrame Spark dans PostgreSQL"""
    url = f"jdbc:postgresql://{POSTGRES['host']}:{POSTGRES['port']}/{POSTGRES['db']}"
    properties = {"user": POSTGRES["user"], "password": POSTGRES["password"], "driver": "org.postgresql.Driver"}
    df.write.jdbc(url=url, table=POSTGRES["table"], mode=mode, properties=properties)
    logging.info(f"✅ Données sauvegardées dans PostgreSQL table: {POSTGRES['table']}")
...
def save_to_local_csv(df, batch_id):
    """Sauvegarde une copie locale du batch nettoyé pour le contrôleur EDA"""
    try:
        output_path = os.path.join(LOCAL_OUTPUT_PATH, f"batch_{batch_id}_clean.csv")
        df.toPandas().to_csv(output_path, index=False)
        logging.info(f"📁 Fichier CSV sauvegardé localement : {output_path}")
    except Exception as e:
        logging.error(f"❌ Erreur sauvegarde CSV local : {e}")


# ================== STREAMING ==================
def process_streaming():
    logging.info("🚀 Démarrage du streaming Spark Cleaner...")
    ...
    try:
        ...
        # --- Sauvegarde PostgreSQL + CSV local ---
        def foreach_batch_function(batch_df, batch_id):
            batch_df_clean = clean_dataframe(batch_df)
            save_to_postgres(batch_df_clean)
            save_to_local_csv(batch_df_clean, batch_id)
...
# ================== MAIN ==================
if __name__ == "__main__":
    logging.info("=== Nettoyage streaming DataFlow360 lancé ===")
    create_db_and_table()
    process_streaming()
